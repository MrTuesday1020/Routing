...
def main():
	...
	if NETWORK_SCHEME == "CIRCUIT":
		for line in lines:
			request = [float(line[0]), line[1], line[2], float(line[3])]
			requests.append(request)
	elif NETWORK_SCHEME == "PACKET":
		interval = 1 / PACKET_RATE
		for line in lines:
			tempTime = 0.0
			while tempTime < float(line[3]):
				request = [float(line[0]) + tempTime, line[1], line[2], interval]
				requests.append(request)
				tempTime += interval
	...
	for request in requests:
		NoOfAllPkt += int(request[3] * PACKET_RATE)
		
		# release sources
		graph.release(request[0])
		...
		if isblock: 
			NoOfBlkPkt += int(request[3] * PACKET_RATE)
		else:
			NoOfSuccReq += 1
			NoOfSuccPkt += int(request[3] * PACKET_RATE)
			NoOfHops += len(path)
			for i in range(len(path)-1):
				PDelays += graph.delay(path[i], path[i+1])
				graph.occupy(path[i], path[i+1], request[0] + request[3])
